...
class N8nCredentialService {
  ...
  Future<N8nCredentialList> getCredentials(
      {int limit = 100, String? cursor}) async {
    try {
      final response =
          await _dio.get('/api/v1/credentials', queryParameters: {
        'limit': limit,
        if (cursor != null) 'cursor': cursor,
      });
      return N8nCredentialList.fromJson(
  ...
  Future<Map<String, dynamic>> getCredentialSchema(String typeName) async {
    try {
      final response =
          await _dio.get('/api/v1/credentials/schema/$typeName');
